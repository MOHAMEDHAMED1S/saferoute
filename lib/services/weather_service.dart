...
class WeatherService {
  ...
  final StreamController<WeatherData> _weatherController = StreamController<WeatherData>.broadcast();
  final StreamController<WeatherForecast> _forecastController = StreamController<WeatherForecast>.broadcast();
  ...
  static const String _apiKey = String.fromEnvironment('WEATHER_API_KEY', defaultValue: ''); // Use environment variable for security
  ...
  Future<void> _cacheWeather() async {
    try {
      ...
      if (_currentWeather != null) {
        await prefs.setString('cached_weather', json.encode(_currentWeather!.toJson()));
      }
      
      if (_currentForecast != null) {
        await prefs.setString('cached_forecast', json.encode(_currentForecast!.toJson()));
      }
    } catch (e) {
  ...
  Future<WeatherForecast> _fetchForecastFromAPI(double latitude, double longitude) async {
    // Simulate API delay
    await Future.delayed(const Duration(seconds: 1));
    ...
    for (int i = 1; i <= 24; i++) {
      ...
      hourlyForecast.add(_generateRealisticWeather(
        latitude, 
        longitude, 
    ...
    for (int i = 1; i <= 7; i++) {
      ...
      dailyForecast.add(_generateRealisticWeather(
        latitude, 
        longitude, 
        timestamp: forecastTime,
        variation: i * 0.2,
  ...
  WeatherData _generateRealisticWeather(
    ...
    WeatherCondition condition = _determineWeatherCondition(
      temperature, humidity, windSpeed, cloudCover, random,
    );
    ...
    } else if (condition == WeatherCondition.rain || condition == WeatherCondition.heavyRain) {
  ...
  double _calculateFeelsLike(double temperature, double humidity, double windSpeed) {
  ...
  WeatherCondition _determineWeatherCondition(
    ...
    if (cloudCover > 80) {
      if (humidity > 80 && random.nextDouble() < 0.4) {
        return random.nextDouble() < 0.7 ? WeatherCondition.rain : WeatherCondition.heavyRain;
      }
  ...
  String _getLocationName(double latitude, double longitude) {
    ...
    } else if (latitude > 22 && latitude < 26 && longitude > 50 && longitude < 57) {
      return 'دولة الإمارات العربية المتحدة';
    } else if (latitude > 25 && latitude < 30 && longitude > 46 && longitude < 49) {
      return 'دولة الكويت';
    } else {
